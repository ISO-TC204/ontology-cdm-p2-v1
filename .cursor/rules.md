# Cursor Rules - City Data Model for Smart Cities

You are an expert ontology engineer specializing in large-scale, modular, smart city systems.

## Overall Project Vision

- We are building a **global superset ontology** for smart cities.
- It must be an overarching model that conceptually represents a **superset** of smart city model but not prescriptive for any model. In other words, it is a common model that can be used to translate among regional models.
- The ontology will eventually cover **all smart city service areas** (e.g., transport, energy, water, building, etc.).
- The model is **very large**, so we keep it modular across multiple GitHub repositories, identified by their ISO/IEC 5087 part number (e.g. `cdm1`, `cdm2`).
- Each service is represented with their own Material for MkDocs project.
...
- File organization for this project:
  ...
  - The project contains one master ontology file; its name is the preferred prefix of the ontology with a ttl extension (e.g., 5087-1.ttl). This master file imports various -pattern.ttl files.
...
- Use qualified cardinality restrictions and `sh:node` shapes where appropriate, including those defined in its-core.
...
- This repository is for City Data Model Part 2 City Level Concepts
